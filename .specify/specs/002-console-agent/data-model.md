...
├── pid, heartbeat_at (for stale-session detection)
├── DialogueTurn (0..n) ── grouped into ──→ ContextBlock (0..n)
├── FocusContext (1)
│   └── PendingDecision (0..n)  [references ApprovalRequestType]
├── ApprovalRequest (0..n)      [own dedicated SQLite table]
...
class SessionState(BaseModel):
    ...
    exit_reason: Optional[str] = None  # "user_exit", "sigterm", "sighup", "sigint", "crash", "stale_reclaimed"
...
- A project MUST NOT have more than one session with `status == active` and a live process (PID running + heartbeat within `heartbeat_timeout_seconds`)
- Stale active sessions (PID not running or heartbeat expired) are auto-reclaimed on startup: status set to `paused`, `exit_reason` set to `stale_reclaimed`
...
class ContextBlock(BaseModel):
    model_config = ConfigDict(exclude={'turns'})           # turns is computed, not persisted
    ...
    def turns(self) -> list[UUID]:
        ...
        Not persisted; DialogueTurn.context_id is authoritative."""
...
- `turns` (computed property, not persisted): Derived at runtime from `dialogue_turns` WHERE `context_id = self.context_id`. The `DialogueTurn.context_id` back-reference is the authoritative source.
...
class PendingDecision(BaseModel):
    ...
    decision_type: ApprovalRequestType   # Reuses the same enum as ApprovalRequest
    description: str
...
CREATE TABLE sessions (
    ...
    pid INTEGER,                               -- OS process ID for stale-session detection
    heartbeat_at TIMESTAMP,                    -- Last heartbeat timestamp
...
-- Approval Requests (dedicated table, not JSON blob)
CREATE TABLE approval_requests (
    ...
    options_json TEXT NOT NULL DEFAULT '[]',   -- [ApprovalOption, ...]
    urgency TEXT DEFAULT 'non_blocking' CHECK (urgency IN ('blocking', 'non_blocking')),
    resolved_at TIMESTAMP,
    user_choice INTEGER,                       -- Index into options_json
    user_comment TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX idx_approval_requests_pending ON approval_requests(session_id) WHERE resolved_at IS NULL;
CREATE INDEX idx_dialogue_turns_session_ts ON dialogue_turns(session_id, timestamp);
...
6. **Per-turn persistence**: Every `DialogueTurn` MUST be persisted to SQLite before the coordinator renders the next response. On graceful exit (`/exit`, SIGTERM, SIGHUP, SIGINT), the session row is finalized and status set to `paused` in a single transaction.

7. **Crash recovery**: On SIGKILL or power loss, the system recovers from the most recent persisted dialogue turn. At most the single unpersisted in-flight turn is lost. Stale active sessions (PID not running or heartbeat expired) are automatically reclaimed on next startup.
