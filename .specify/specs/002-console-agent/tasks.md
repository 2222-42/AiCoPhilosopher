...
- [ ] T-006 [P] [Foundation] Write unit tests for SQLite session schema creation and constraints
...
- [ ] T-007 [Foundation] Add session persistence method signatures to StoragePort protocol
  - **Files**: `src/aicophilosopher/ports/storage_port.py` (modify: add abstract method signatures)
  - **Methods to add**: `save_session(session: SessionState) -> None`, `load_session(project_id: UUID) -> SessionState | None`, `list_projects_with_sessions() -> list[dict]`, `reclaim_stale_sessions() -> int`, `save_dialogue_turn(turn: DialogueTurn, session_id: UUID) -> None`, `save_approval_request(request: ApprovalRequest, session_id: UUID) -> None`, `load_pending_approvals(session_id: UUID) -> list[ApprovalRequest]`, `update_session_heartbeat(session_id: UUID) -> None`, `finalize_session(session_id: UUID, reason: str) -> None`
  - **AC**: All 9 method signatures use `@abstractmethod` with full type annotations; each has docstring describing behavior per `data-model.md` invariants; `mypy --strict src/aicophilosopher/ports/storage_port.py` passes (catches any implementation that misses a method); existing adapter tests still pass (mock adapter implementing partial StoragePort should show `TypeError` if methods missing); no implementation — signatures only
  - **Depends on**: T-003 (needs entity types for signatures)
...
- [ ] T-011 [US1] Implement REPL main loop in `src/aicophilosopher/presentation/repl.py`
  ...
  - **AC**: `run_repl(project_id: UUID | None = None, test_mode: bool = False) -> None` function with: prompt_toolkit `PromptSession` with `FileHistory`; input routing: `/` prefix → `slash_commands.dispatch()` else → `nlu.classify_intent()` → `coordinator.process()`; Ctrl+D handler → graceful exit; Ctrl+C handler → graceful exit; progressive disclosure rendering via `rendering.render_response()`; command history (up/down arrows, Ctrl+R search) works; session heartbeat sent before every prompt display; workstream status queue flushed after each response render; test_mode flag: when True, uses mock coordinator and skips LLM calls; exit flow: finalize session via `session_manager.finalize()`, save command history, print goodbye; `ruff check` passes; `mypy` passes; imports only from `presentation/`, `application/`, `domain/`, `ports/` (no direct infrastructure imports)
  - **Depends on**: T-010 (TDD: tests before implementation), T-009 (NLU classifier)
...
- [ ] T-024 [US4] Write unit tests for REPL-to-Coordinator integration
  ...
  - **AC**: ≥15 tests: test `start_inquiry` intent → coordinator.engage_socratic_dialogue() called; test `clarify_question` intent → coordinator.process_clarification() called; test `propose_workstream` intent → coordinator.propose_workstreams() called with refined goals; test `approve_action` with workstream proposal → coordinator.launch_workstream() called; test `steer_workstream` intent → coordinator.steer_workstream(ws_id, instruction) called; test `request_status` intent → coordinator.get_status() called; test `request_export` intent → coordinator.export_document(format) called; test `inject_information` (PDF upload) → coordinator.ingest_pdf(path) called; test `compare_traditions` intent → coordinator.launch_cross_traditional_comparison() called; test coordinator response transforms to CoordinatorResponse for rendering; test workstream status updates queued when received mid-input; test bidirectional help: coordinator raises approval request → surfaced as ⚠️ in Summary; test user's approval response routes back to coordinator; test existing message protocol used (no new message types introduced); all tests mock coordinator; `pytest tests/unit/presentation/test_coordinator_integration.py -v` passes
...
- [ ] T-025 [US4] Implement REPL-Coordinator adapter in `src/aicophilosopher/presentation/repl.py`
  ...
  - **AC**: `_route_to_coordinator(intent: UserIntent, coordinator: ProjectCoordinator) -> CoordinatorResponse` function; maps all 16 intent types to coordinator method calls; converts coordinator output to `CoordinatorResponse` for rendering; `approval_request` extraction: when coordinator output contains approval request, wrap in `CoordinatorResponse(is_approval_request=True, approval_options=[...])`; workstream status changes from async updates queued and surfaced per FR-024; uses existing JSON message protocol from 001 contracts (FR-023); no new message types introduced; all tests from T-024 pass; no regressions in US1-US3 tests
